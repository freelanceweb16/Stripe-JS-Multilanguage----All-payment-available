...
(async () => {
  ...
  const handlePayment = (paymentResponse) => {
    ...
    if (error && error.type === 'validation_error') {
	  ...
	  setTimeout(function(){
		  window.location.href = "https://www.mundicontest.com/acheter-des-contests/?contest="+getVariableUrlId()+"&status=confirmed";
	  }, 2000);
  ...
  const updateButtonLabel = (paymentMethod, bankName) => {
	...
	if (paymentMethod !== 'card') {label = `Pay ${amount} with ${name}`;}
    if (paymentMethod === 'wechat') {label = `Generate QR code to pay ${amount} with ${name}`;}
    if (['sepa_debit', 'au_becs_debit'].includes(paymentMethod) && bankName) {label = `Debit ${amount} from ${bankName}`;}
	
	if(getVariableUrlLanguage() == 'en'){}else{
		const xhttp = new XMLHttpRequest();
		xhttp.overrideMimeType("application/json");
		xhttp.onload = function() {
			var jsonResponse = JSON.parse(xhttp.responseText);
			for(i=0;i<=(jsonResponse['i18n'].length -1);i++){
				if(jsonResponse['i18n'][i]['selecteur'] != ''){
					if(jsonResponse['i18n'][i]['selecteur'] == 'payOrder'){
						label = jsonResponse['i18n'][i]['traduction']+` ${amount}`
						if (paymentMethod !== 'card') {label = jsonResponse['i18n'][i]['traduction']+` ${amount}`;}
						if (paymentMethod === 'wechat') {label = jsonResponse['i18n'][i]['traductionWechat']+` ${amount}`;}
						if (['sepa_debit', 'au_becs_debit'].includes(paymentMethod) && bankName) {
							label = jsonResponse['i18n'][i]['traductionDebit1']+` ${amount}`+jsonResponse['i18n'][i]['traductionDebit2']+` ${bankName}`;
						}
						updateSubmitButtonPayText(label);
					}
					// $('#'+jsonResponse['i18n'][i]['selecteur']+'').html(jsonResponse['i18n'][i]['traduction']);
				}else{}
			}
		}
		xhttp.open("GET", "/javascripts/i18n/"+getVariableUrlLanguage()+".json");
		xhttp.send();
...
function getVariableUrlLanguage(){
	const queryStringUrl = window.location.search;
	const urlParams2 = new URLSearchParams(queryStringUrl);
	return urlParams2.get('lang');
}

function getVariableUrlId(){
	const queryStringUrl = window.location.search;
	const urlParams2 = new URLSearchParams(queryStringUrl);
	return urlParams2.get('id');
}


// const currentLanguage = "<a href='?id="+getVariableUrlId()+"&lang="+getVariableUrlLanguage()+"'><img src=''></a>";
const frenchLanguage = "<a href='?id="+getVariableUrlId()+"&lang=fr'><img src='/images/fr.png' height='16px'></a>";
const italianLanguage = "<a href='?id="+getVariableUrlId()+"&lang=it'><img src='/images/it.png' height='16px'></a>";
const spainLanguage = "<a href='?id="+getVariableUrlId()+"&lang=es'><img src='/images/es.png' height='16px'></a>";
const portugalLanguage = "<a href='?id="+getVariableUrlId()+"&lang=pt'><img src='/images/pt.svg' height='16px'></a>";
const englishLanguage = "<a href='?id="+getVariableUrlId()+"&lang=en'><img src='/images/en.png' height='16px'></a>";
const deutchLanguage = "<a href='?id="+getVariableUrlId()+"&lang=de'><img src='/images/de.svg' height='16px'></a>";
const netherlandsLanguage = "<a href='?id="+getVariableUrlId()+"&lang=nl'><img src='/images/nl.png' height='16px'></a>";

let selectorLanguage = frenchLanguage+'&nbsp;&nbsp;'+italianLanguage+'&nbsp;&nbsp;'+spainLanguage+'&nbsp;&nbsp;'+portugalLanguage+'&nbsp;&nbsp;'+englishLanguage+'&nbsp;&nbsp;'+deutchLanguage+'&nbsp;&nbsp;'+netherlandsLanguage;

$(function() {
	
	$('#languageSelector').html(selectorLanguage);
	
	/* AFFICHAGE DE LA LANGUE SELECTIONNE */
	if(getVariableUrlLanguage() == 'en'){}else{
		try {
			const xhttp = new XMLHttpRequest();
			xhttp.overrideMimeType("application/json");
