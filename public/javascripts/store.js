...
class Store {
  ...
  getPaymentTotal() {
    return Object.values(this.lineItems).reduce(
      (total, {product, sku, quantity}) =>
        total + this.products[product].skus.data[0].price,
      0
    );
  }
  ...
  async displayPaymentSummary() {
    ...
    for (let [id, product] of Object.entries(this.products)) {
      ...
      const quantity = 1;
	  let productIdSub = 580;
	  
		var $_GET = {};
		if(document.location.toString().indexOf('?') !== -1) {
			const queryString = window.location.search;
			const urlParams = new URLSearchParams(queryString);
			productIdSub = urlParams.get('id');
	  ...
	  if(product.id == productIdSub){
		  let lineItem = document.createElement('div');
		  lineItem.classList.add('line-item');
		  lineItem.innerHTML = `
			<img class="image" src="${product.images[0]}" alt="${product.name}" width="100%" height="auto">
			<div class="label">
